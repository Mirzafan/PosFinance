  ...
  return (
    <DashboardLayout>
      ...
      <div className="space-y-6 animate-fadeIn">
        ...
        <div className="bg-[#0B101B] border border-[#182232] rounded-2xl p-6 sm:p-8 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-80 h-80 bg-orange-500/5 rounded-full blur-3xl pointer-events-none" />

          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 relative z-10">
            <div className="flex items-center gap-5">
              {/* Avatar Icon */}
              <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-tr from-orange-600 to-amber-500 flex items-center justify-center text-white font-bold text-2xl sm:text-3xl shadow-xl shadow-orange-950/50 shrink-0 border border-orange-400/30">
            ...
            {/* Navigation Tabs */}
            <div className="flex items-center bg-slate-950/80 p-1.5 rounded-xl border border-slate-800 self-stretch sm:self-auto justify-center">
              <button
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-[#0B101B] border border-[#182232] rounded-2xl p-5 relative overflow-hidden">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Total Pencatatan</span>
                ...
                <div className="space-y-1">
                  <h3 className="text-2xl font-bold text-white tracking-tight">
                    {financialStats.total_input} <span className="text-xs font-normal text-slate-400">Transaksi</span>
                  </h3>
              ...
              <div className="bg-[#0B101B] border border-[#182232] rounded-2xl p-5 relative overflow-hidden space-y-3">
                <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Volume Akumulasi Kas</span>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <span className="text-[10px] text-emerald-400 font-semibold flex items-center gap-1">
                      <ArrowUpRight className="h-3 w-3" /> Pemasukan
                    </span>
                    <p className="text-sm font-bold text-white mt-0.5">{formatRupiah(financialStats.total_pemasukan)}</p>
                  </div>
                  <div>
                    <span className="text-[10px] text-rose-400 font-semibold flex items-center gap-1">
                      <ArrowDownLeft className="h-3 w-3" /> Pengeluaran
                    </span>
                    <p className="text-sm font-bold text-white mt-0.5">{formatRupiah(financialStats.total_pengeluaran)}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#0B101B] border border-[#182232] rounded-2xl p-5 relative overflow-hidden space-y-3">
                <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Status Approval Transaksi</span>
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-2">
                    <span className="text-xs font-bold text-emerald-400 block">{financialStats.approved_count}</span>
                  ...
                  <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-2">
                    <span className="text-xs font-bold text-amber-400 block">{financialStats.pending_count}</span>
                    <span className="text-[10px] text-slate-400">Pending</span>
                  </div>
                  <div className="bg-rose-500/10 border border-rose-500/20 rounded-xl p-2">
                    <span className="text-xs font-bold text-rose-400 block">{financialStats.rejected_count}</span>
                    <span className="text-[10px] text-slate-400">Ditolak</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#0B101B] border border-[#182232] rounded-2xl p-6 relative overflow-hidden space-y-4">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">Daftar 10 transaksi arus kas terbaru yang diinput oleh akun ini</p>
              </div>

              <div className="overflow-x-auto">
                <table className="w-full text-xs text-left border-collapse">
                  <thead>
                    <tr className="border-b border-slate-800/80 text-slate-400 font-semibold text-[11px] uppercase tracking-wider bg-slate-950/40">
                        ...
                        return (
                          <tr key={trx.id} className="text-slate-300 hover:bg-slate-800/30 transition-colors">
                            ...
                            <td className="py-3 px-2 whitespace-nowrap">
                              <span className="inline-block text-[10px] font-bold text-slate-300 bg-slate-800/80 px-2 py-0.5 rounded-full border border-slate-700/50">
                            ...
                            <td className="py-3 px-2 text-xs max-w-[150px] truncate text-slate-300">
                              {trx.keterangan || '-'}
                            </td>
        ...
        {activeTab === 'settings' && (
          <div className="space-y-6 animate-fadeIn">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-[#0B101B] border border-[#182232] rounded-2xl p-6">
            ...
            <div className="bg-[#0B101B] border border-rose-900/30 rounded-2xl p-6">
              <DeleteUserForm />
            </div>
          </div>
        )}
