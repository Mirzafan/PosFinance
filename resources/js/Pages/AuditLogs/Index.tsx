  ...
  return (
    <DashboardLayout>
      ...
      <div className="space-y-6 animate-fadeIn">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-[#0B101B] border border-[#182232] rounded-2xl p-5 relative overflow-hidden">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Total Audit Log</span>
              <div className="p-2.5 rounded-xl bg-orange-500/10 border border-orange-500/20 text-orange-400">
                <ShieldCheck className="h-5 w-5" />
              </div>
            </div>
            <h3 className="text-2xl font-bold text-white tracking-tight">{stats.total_logs}</h3>
            <p className="text-[11px] text-slate-500 mt-1">Total riwayat aktivitas tercatat</p>
          </div>

          <div className="bg-[#0B101B] border border-[#182232] rounded-2xl p-5 relative overflow-hidden">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Log Transaksi</span>
              <div className="p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-400">
                <Coins className="h-5 w-5" />
              </div>
            </div>
            <h3 className="text-2xl font-bold text-white tracking-tight">{stats.transaksi_count}</h3>
            <p className="text-[11px] text-slate-500 mt-1">Aktivitas input & edit transaksi</p>
          </div>

          <div className="bg-[#0B101B] border border-[#182232] rounded-2xl p-5 relative overflow-hidden">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Persetujuan (Approval)</span>
              <div className="p-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400">
                <CheckCircle2 className="h-5 w-5" />
              </div>
            </div>
            <h3 className="text-2xl font-bold text-white tracking-tight">{stats.approval_count}</h3>
            <p className="text-[11px] text-slate-500 mt-1">Status disetujui & ditolak</p>
          </div>

          <div className="bg-[#0B101B] border border-[#182232] rounded-2xl p-5 relative overflow-hidden">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Log Pengguna</span>
              <div className="p-2.5 rounded-xl bg-purple-500/10 border border-purple-500/20 text-purple-400">
                <Users className="h-5 w-5" />
              </div>
            </div>
            <h3 className="text-2xl font-bold text-white tracking-tight">{stats.user_count}</h3>
            <p className="text-[11px] text-slate-500 mt-1">Pengelolaan akun pengguna</p>
          </div>
        </div>

        {/* Filter Section */}
        <div className="bg-[#0B101B] border border-[#182232] rounded-2xl p-5 space-y-4">
          <form onSubmit={handleFilter} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            ...
            <div className="lg:col-span-2 relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-orange-500"
              />
            </div>
            ...
            <div>
              <select
                ...
                className="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-300 focus:outline-none focus:ring-1 focus:ring-orange-500 cursor-pointer"
              >
        ...
        {/* Audit Log Table */}
        <div className="bg-[#0B101B] border border-[#182232] rounded-2xl overflow-hidden shadow-xl">
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-800 text-slate-400 font-semibold text-[11px] uppercase tracking-wider bg-slate-950/60">
                      ...
                      <tr key={log.id} className="text-slate-300 hover:bg-slate-800/30 transition-colors">
                        ...
                        <td className="py-3.5 pl-6 pr-3 whitespace-nowrap">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <p className="font-bold text-white text-xs leading-tight">{formattedTime} <span className="text-[10px] text-slate-500">WIB</span></p>
                        ...
                        {/* Deskripsi */}
                        <td className="py-3.5 px-3 text-xs text-slate-200 leading-relaxed font-sans">
                          {log.description}
                        </td>
          ...
          {/* Pagination */}
          {logs.last_page > 1 && (
            <div className="flex items-center justify-between px-6 py-4 border-t border-slate-800/80 bg-slate-950/40 text-xs">
              <span className="text-slate-400">
                Menampilkan <span className="font-semibold text-white">{logs.data.length}</span> dari <span className="font-semibold text-white">{logs.total}</span> data log
              </span>
